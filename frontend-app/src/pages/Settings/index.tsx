...
import { Bot, Cpu, Palette, Globe, Shield, User, Mic } from 'lucide-react';
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <input type="range" min={80} max={260} value={botSize}
                onChange={e => {
                  ...
                  (window as any).__botSizeTimer = setTimeout(() => {
                    const token = localStorage.getItem('token');
                    if (token) fetch('/api/v1/bot/preferences', { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ bot_size: v }) }).catch(() => {});
                  }, 500);
                }}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
        ...
        {!hasBrowserSTT && <span className="font-mono" style={{ fontSize: 9, color: '#c0392b' }}>(STT not supported in this browser)</span>}
      </label>

      {config.enabled && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="font-mono" style={{ fontSize: 10, color: 'var(--dim)', minWidth: 80 }}>TTS Provider</span>
            <select value={config.ttsProvider} onChange={e => save({ ttsProvider: e.target.value })}
              className="font-mono" style={{ fontSize: 11, padding: '4px 8px', border: '1.5px solid var(--line)', background: 'var(--cream)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="font-mono" style={{ fontSize: 10, color: 'var(--dim)', minWidth: 80 }}>Language</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="font-mono" style={{ fontSize: 10, color: 'var(--dim)', minWidth: 80 }}>Speed</span>
            <input type="range" min={0.5} max={2.0} step={0.1} value={config.rate}
              onChange={e => save({ rate: parseFloat(e.target.value) })}
              style={{ flex: 1, maxWidth: 150, accentColor: 'var(--orange)' }} />
            <span className="font-mono" style={{ fontSize: 10, color: 'var(--dim)' }}>{config.rate}x</span>
          </div>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            ...
            <span className="font-mono" style={{ fontSize: 11, color: 'var(--mid)' }}>Auto-speak bot responses</span>
          </label>
        </>
      )}
    </div>
